...
const clienteSchema = Joi.object({
    cpf: Joi.string().length(11).required(),//cpf deve ser uma string(por causa do json) e ter 11 caracteres
    nome: Joi.string().required().max(50),// nome deve ser um texto e ser obrigatorio ser preenchido, min:tamanho minimo de caracter
    endereco: Joi.string().required(),
    bairro: Joi.string().required(),
    cidade: Joi.string().allow(''),
    telefone: Joi.string().required().max(100),//max:tamanho maximo de caracter
